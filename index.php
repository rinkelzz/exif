...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors) && $uploadedFilePath !== null) {
        [$metadata, $metadataErrors] = extractMetadata($uploadedFilePath, $uploadedMime);
...
function extractMetadata(string $filePath, ?string $mime): array
{
    $metadata = [];
    $errors = [];

    if (function_exists('exif_read_data')) {
        $metadata = @exif_read_data($filePath, null, true) ?: [];
        if (!empty($metadata)) {
            return [$metadata, []];
        }
    } else {
    ...
    if (class_exists('Imagick')) {
        try {
            ...
            if (!empty($properties)) {
                ksort($properties);
                return [[translateImagickPropertiesLabel($mime) => $properties], []];
            }
        } catch (Throwable $exception) {
            $errors[] = 'Metadaten konnten nicht mit Imagick gelesen werden.';
        }
    }
...
function translateImagickPropertiesLabel(?string $mime): string
{
    if ($mime !== null && stripos($mime, 'heic') !== false) {
        return 'HEIC-Metadaten';
    }

    if ($mime !== null && stripos($mime, 'heif') !== false) {
        return 'HEIF-Metadaten';
    }

    return 'Bildinformationen';
}
...
{
    if (class_exists('Imagick')) {
        $imagickThumbnail = generateImagickThumbnail($filePath);
        if ($imagickThumbnail !== null) {
            return $imagickThumbnail;
        }
    }
...
function generateImagickThumbnail(string $filePath): ?array
{
    try {
        $imagick = new Imagick();
        $imagick->readImage($filePath);
        ...
        if (!$imagick->thumbnailImage(200, 0)) {
            $imagick->clear();
            $imagick->destroy();
            return null;
        }

        if (method_exists($imagick, 'setImageFormat')) {
        ...
        $generated = $imagick->getImageBlob();
        $imagick->clear();
        $imagick->destroy();
...
function determineImageMimeType(string $filePath): array
{
    ...
    if (class_exists('Imagick')) {
        try {
            ...
            $mapped = mapImagickFormatToMime($format);
            if ($mapped !== null) {
                return [true, $mapped];
            }
        } catch (Throwable $exception) {
...
function mapImagickFormatToMime(string $format): ?string
{
